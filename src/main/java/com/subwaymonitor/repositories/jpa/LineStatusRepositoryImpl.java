...
public class LineStatusRepositoryImpl implements LineStatusRepository {
    ...
    public List<LineStatus> listAllLinesByVerificationNumber(Integer verificationNumber) {
        try {
            StringBuilder query = new StringBuilder("FROM LineStatusEntity ls ")
            ...
            List<LineStatusEntity> lineStatusEntities
                    = this.entityManager.createQuery(query.toString(), LineStatusEntity.class)
                                        .setParameter("verificationNumber", verificationNumber)
                                        .getResultList();

            return lineStatusEntities.stream().map(LineStatusEntity::convert).collect(Collectors.toList());
        } catch (Exception e) {
            throw new DatabaseException(e);
        }
    }
    ...
    public Integer findLastVerification() throws NotFoundException {
        try {
            StringBuilder query = new StringBuilder("SELECT verificationNumber FROM LineStatusEntity ")
                    .append("ORDER BY verificationNumber DESC");

            return this.entityManager.createQuery(query.toString(), Integer.class)
                          .setMaxResults(1)
